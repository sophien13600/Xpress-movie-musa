...
const addFavorie=async(req, res, next)=>{   
    try {
        const film = await favoriRepository.findFavoriFilmById(req.body.filmId, req.body.userId)
        if (film[0]==null) {
            const favoriFilm = await favoriRepository.addFavorie(req.body.filmId, req.body.userId)
