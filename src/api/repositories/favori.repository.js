...
const findFavoriFilmById = async (id, user_id) => {
  try {
    const SELECT =
      "SELECT * FROM films JOIN favoris as f on f.film_id = films.id where films.id =? and f.user_id=?; ";
...
const findUserFavori = async (user_id) => {
  try {
    const SELECT =
      "SELECT * FROM films JOIN favoris as f on f.film_id = films.id where f.user_id =?; ";
